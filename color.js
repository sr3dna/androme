...
for (var i in COLOR_X11_CSS3) {
    for (var j in COLOR_X11_CSS3[i]) {
        COLOR_X11_CSS3[i].rgb = convertHextoRGB(COLOR_X11_CSS3[i][j]);
        COLOR_X11_CSS3[i].hsl = convertRGBtoHSL(COLOR_X11_CSS3[i].rgb.r, COLOR_X11_CSS3[i].rgb.g, COLOR_X11_CSS3[i].rgb.b);
        COLOR_SORT_HSL.push({ name: i, hex: COLOR_X11_CSS3[i].hex, hsl: COLOR_X11_CSS3[i].hsl });
    }
}

COLOR_SORT_HSL.sort(sortHSL);

function sortHSL(a, b) {
    var [c, d] = [a.hsl.h, b.hsl.h];
    if (c == d) {
        [c, d] = [a.hsl.s, b.hsl.s];
        if (c == d) {
            [c, d] = [a.hsl.l, b.hsl.l];
...
function findNearestColor(value) {
    ...
    if (hsl) {
        ...
        result.sort(sortHSL);
        var index = result.findIndex(item => item.name == '');
